...
export const App: React.FC<AppProps> = ({client}) => {
  ...
  return (
    <>
      ...
      <>{authenticationStatus === AuthenticationStatus.UNCERTAIN ? <div>Uncertain User</div> : authenticationStatus === AuthenticationStatus.FALSE ? <div>
      Not Logged In
      </div> : authenticationStatus === AuthenticationStatus.TRUE ? <div>Logged In</div> : <></>}</>
    </>
  );
};
